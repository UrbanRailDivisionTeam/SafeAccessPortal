  ...
  try {
    ...
    try {
      ...
      if (suggestions && search) {
        ...
        const query = 'SELECT id, name FROM projects WHERE name ILIKE $1 ORDER BY name LIMIT $2'
    ...
    try {
      if (ids) {
        ...
        const result = await client.query(
          `DELETE FROM projects WHERE id IN (${placeholders}) RETURNING id`,
          idArray
        )
